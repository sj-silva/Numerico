...
void init_OPENGL(GLFWwindow* window)
{
    ...
    nodes.CreateNodes(g,XMIN,XMAX,EQUALLY);
...
void desenha()
{
    ...
    if (show_quad_spline) {
        ...
        quad.PlotQSpline(qsplineColor);
        glDisable(GL_LINE_STIPPLE);
    }


    // CUBIC SPLINE
    if (show_cubic_spline) {
        glEnable(GL_LINE_STIPPLE);
        short padrao = 0x5555;
        glLineStipple(3, padrao);
        glm::vec3 csplineColor(0.709, 0.870, 0.792);
        cubic.PlotCSpline(csplineColor);
        glDisable(GL_LINE_STIPPLE);
    }

    // PLOT POINTS
    if (show_points) {
        glm::vec3 pointColor(1.f, 0.f, 0.f);
        nodes.PlotPoints(pointColor);
    }
